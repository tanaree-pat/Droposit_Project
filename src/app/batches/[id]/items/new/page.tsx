...
/**
 * Create new item — mockup hero pattern + image capture/upload control.
 * The image picker uses a native file input with `capture="environment"`
 * so mobile browsers open the rear camera directly when tapped.
 */
export default function NewItemPage() {
  ...
  const submit = (e: React.FormEvent) => {
    ...
    setLoading(true);
    setTimeout(() => router.push(`/batches/${id}`), 600);
  };
  ...
  return (
    <MobileShell>
      ...
      <form onSubmit={submit} className="mx-5 -mt-6 surface-card p-6 flex flex-col gap-5">
        ...
        <Field
          label="Description"
          hint="Helpful details: color, material, monogram, scratches."
        >
