...
/**
 * Depositor QR screen — per-batch QR codes.
 * Each QR encodes `droposit://scan/{qr_token}` which the staff scanner
 * resolves via GET /scan/:qr_token → POST /scan/:qr_token/deposit|checkout.
 *
 * Deep-linked from batch detail via ?batch=<id>; falls back to the first
 * non-claimed batch when accessed from the nav tab.
 */
export default function QRPage() {
  const sp = useSearchParams();
  const preselect = sp.get("batch");

  const activeBatches = batches.filter((b) => b.status !== "claimed");

  const defaultId =
    preselect ?? activeBatches[0]?.id ?? batches[0]?.id ?? "";
  const [selectedId, setSelectedId] = React.useState(defaultId);

  const batch = batches.find((b) => b.id === selectedId) ?? batches[0];
  const qrValue = `droposit://scan/${batch.qr_token}`;
        ...
        <div className="flex flex-col items-center gap-6">
          ...
          <div className="surface-card w-full p-4 flex flex-col gap-3">
            ...
            <div className="flex items-center justify-between text-small">
              ...
              <span className="font-mono text-gray-300 text-caption tracking-wider">
                {batch.qr_token}
              </span>
            </div>
            <div className="flex items-center justify-between text-small">
...
function ActionTile({
  ...
  return (
    <Button
      ...
      size="md"
      fullWidth
      className="flex-col h-20 gap-1 rounded-lg"
    >
